...
export async function tossAPod({ walletConnector, gps, account }) {
  ...
  // signTransaction expects an array of transaction groups.
  const [signed] = await walletConnector.signTransaction([[{ txn, signers: [account] }]]);
  const send = await algod.sendRawTransaction(signed).do();
  return send.txId;
}
